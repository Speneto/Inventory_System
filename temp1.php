...
$sqlc = @mysql_query("SELECT cashdiscount from salesdiscount where Date ='$date'");
...
$sqld = @mysql_query("SELECT debitdiscount from salesdiscount where Date ='$date'");
...
/*$sql10 = @mysql_query("SELECT debitdiscount from salesdiscount");
while($rww = @mysql_fetch_array($sql10)){
$debitdiscount = $rww['debitdiscount'];
}*/


$sql1 = @mysql_query("SELECT sum(Amount) FROM debitsales where Date='$date'");
while($row1 = @mysql_fetch_array($sql1))
{
    $sum=$row1['sum(Amount)'];
...
$spreadsheet->getActiveSheet()->getStyle('B1')->getFont()->setSize(29);
...
$worksheet->setCellValue('B1','NOBLE ALCHUKS NIGERIA LTD');//Title
